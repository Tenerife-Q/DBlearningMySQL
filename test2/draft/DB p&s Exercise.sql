...
-- 1. 存储过程用 CALL 调用，存储函数用 SELECT 调用        _______
-- 2. 存储过程可以有多个返回值（通过 OUT 参数）           _______
-- 3. 存储函数必须有且只有一个 RETURN 语句                _______
-- 4. 存储函数可以有 OUT 参数                             _______
-- 5. 存储函数可以在 SQL 语句中使用（如 SELECT, WHERE）   _______
-- 6. 存储过程可以在 SELECT 语句中直接调用                _______
...
-- 1. 游标必须在使用前 OPEN，使用后 CLOSE               _______
-- 2. FETCH 每次读取一行数据                            _______
-- 3. 游标关闭后可以直接再次 FETCH                      _______
-- 4. 一个存储过程中可以声明多个游标                    _______
-- 5. 游标只能在存储过程或存储函数中使用                _______
...
-- 1. DECLARE v_count INT DEFAULT 0;       类型：_______
-- 2. SET @result = 100;                   类型：_______
-- 3. SELECT @@autocommit;                 类型：_______
-- 4. SET @@global.max_connections = 200;  类型：_______
